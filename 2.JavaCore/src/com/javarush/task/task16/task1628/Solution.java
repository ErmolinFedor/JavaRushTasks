...
public class Solution {
    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            while (!isInterrupted()){
                try {
                    if (reader.ready()) { //проверка считывания строки, чтобы не было лишнего ожидания (есть что читать?)
                        result.add(reader.readLine()); // считываем из консоли и записываем значение в List
                        readStringCount.getAndIncrement(); //увеличиваем на 1 текущее значений
                    }
